...
export default class ElevenlabsConnector extends TTSConnectorInterface {
    ...
    constructor(mainModule, logger) {
        super(mainModule, logger);
    }
    ...
    getSettingsSchema() {
        return [
            { key: ELEVENLABS_CONSTANTS.APIKEY, name: "acd.ta.settings.ApiKey", hint: "acd.ta.settings.ApiKeyHint", scope: "client", config: true, type: String, default: "", requiresReload: true },
            { key: ELEVENLABS_CONSTANTS.MASTERAPIKEY, name: "acd.ta.settings.MasterApiKey", hint: "acd.ta.settings.MasterApiKeyHint", scope: "world", config: true, type: String, default: "", requiresReload: true },
    ...
    registerAdditionalSettings() {
        ...
        if (!game.settings.settings.has(`${this.mainSettingsId}.${ELEVENLABS_CONSTANTS.DEFAULT_MODEL}`)) {
            game.settings.register(this.mainSettingsId, ELEVENLABS_CONSTANTS.DEFAULT_MODEL, {
                ...
                type: String,
                choices: this.settingsModelChoices,
            });
        }

        if (!game.settings.settings.has(`${this.mainSettingsId}.${ELEVENLABS_CONSTANTS.DEFAULT_LANGUAGE}`)) {
            game.settings.register(this.mainSettingsId, ELEVENLABS_CONSTANTS.DEFAULT_LANGUAGE, {
                name: "acd.ta.settings.DefaultLanguage",
                hint: "acd.ta.settings.DefaultLanguageHint",
                scope: "client",
                config: true,
                default: "en",
                type: String,
                choices: this.settingsLanguageChoices,
            });
        }

    }

    get settingsModelChoices() {
        let modelChoices = {};
    ...
    get settingsLanguageChoices() {
        let languageChoices = {};
        this.models.forEach(m => {
            m.languages.forEach(l => {
                languageChoices[l.language_id] = `${l.name}`;
            });
        });
    ...
    async initializeModels() {
        ...
        try {
            const remoteModels = await new api.ListModelsRequest(this).fetch();
    ...
    async initializeAvailableVoices() {
        ...
        try {
            const remoteVoices = await new api.GetVoicesRequest(this).fetch();
    ...
    async initializeUserdata() {
        ...
        try {
            this.subscriptionInfo = await new api.GetUserSubscriptionInfoRequest(this).fetch();
        } catch (err) {
    ...
    async openVoiceSettingsApp(actors) {
        const app = new VoiceSettingsApp(this, actors);
        app.render(true);
    }
    ...
    retrieveLanguageId(actor, modelId) {
        ...
        if (languageId == null || !this.models.find(m => m.languages.find(l => l.language_id === languageId))) {
            ...
            languageId = game.settings.get(this.mainModule.id, ELEVENLABS_CONSTANTS.DEFAULT_LANGUAGE) || null;
    ...
    retrieveModelId(actor) {
        ...
        if (modelId == null || !this.models.find(m => m.model_id === modelId)) {
            ...
            modelId = game.settings.get(this.mainModule.id, ELEVENLABS_CONSTANTS.DEFAULT_MODEL) || null;
